...
export function normalizeRow(reportType, header, values, context, headerResolution) {
  ...
  if (canonicalReportType === REPORT_TYPES.DLOB) {
    return {
      ...
      year_scope: context.yearScope,
      ordem_bancaria:
        context.dlobFillDown?.ordem_bancaria ??
        toText(getCanonicalValue(rawRow, headerResolution, "ordem_bancaria")),
      data_pagamento:
        context.dlobFillDown?.data_pagamento ??
        parseDate(getCanonicalValue(rawRow, headerResolution, "data_pagamento")),
      documento_liquidacao: toText(getCanonicalValue(rawRow, headerResolution, "documento_liquidacao")),
      codigo_unidade_gestora: toText(getCanonicalValue(rawRow, headerResolution, "codigo_unidade_gestora")),
      nome_usuario_criou: toText(getCanonicalValue(rawRow, headerResolution, "nome_usuario_criou")),
...
export function normalizeRows(reportType, header, rows, context, headerResolution) {
  ...
  if (canonicalReportType === REPORT_TYPES.DLOB) {
    let lastOrdemBancaria = null;
    let lastDataPagamento = null;

    return rows.map((values, index) => {
      const rawRow = mapRowFromHeader(header, values);
      const currentOrdemBancaria = toText(
        getCanonicalValue(rawRow, headerResolution, "ordem_bancaria")
      );
      const rawDataPagamento = toText(
        getCanonicalValue(rawRow, headerResolution, "data_pagamento")
      );
      const currentDataPagamento = parseDate(rawDataPagamento);

      if (currentOrdemBancaria) {
        lastOrdemBancaria = currentOrdemBancaria;
      }

      if (rawDataPagamento) {
        lastDataPagamento = currentDataPagamento;
      }
      ...
      return normalizeRow(
        ...
        values,
        {
          ...
          rowNumber: index + 2,
          dlobFillDown: {
            ordem_bancaria: currentOrdemBancaria ?? lastOrdemBancaria,
            data_pagamento: rawDataPagamento ? currentDataPagamento : lastDataPagamento
          }
        },
        headerResolution
      );
