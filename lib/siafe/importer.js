...
function getSafeBatchSeed(reportType, yearScope) {
  return {
    report_type: REPORT_SCHEMAS[reportType] ? reportType : "NE",
...
function mapBatchResponse(batch, warnings = []) {
  return {
    id: batch.id,
    status: batch.status,
...
async function createFailedBatch(supabase, seed, payload) {
  await createBatchRecord(supabase, {
    ...
    ...(payload.importedBy ? { imported_by: payload.importedBy } : {}),
  });
}
...
export async function finalizeBatchSuccess({
  ...
  // Todos os year_scopes usam a mesma lógica atômica: desativa o batch
  // anterior, apaga suas linhas normalizadas e ativa o novo batch.
  ...
  if (error) {
    throw new Error(`Failed to finalize import: ${error.message}`);
  }
...
export async function processSiafeUpload({
  ...
  reportType,
  yearScope,
  importedBy    = null,   // UUID do usuário admin responsável
  supabaseClient,
}) {
  ...
  try {
    ...
    const headerResolution = resolveHeaders(reportType, header);
    structuralWarnings = headerResolution.warnings;
    ...
    return mapBatchResponse(batch, structuralWarnings);
  } catch (error) {
