...
async function fetchCpagData(supabase) {
  const [obAggResult, dlAggResult, liquidadosResult, monitoramentoResult] = await Promise.all([
    ...
    supabase
      ...
      .select("numero_processo, codigo_nota_empenho, documento_liquidacao, credor, codigo_natureza_despesa, fonte, valor_liquido, valor_bruto, valor_liquidado_a_pagar")
      .order("documento_liquidacao", { ascending: false })
      .limit(100),
    supabase
      .from("vw_monitoramento_pagamentos")
      .select("numero_processo, credor, fonte, documento_liquidacao, ordem_bancaria, data_pagamento, valor, confirmado_manualmente, confirmado_por, confirmado_em, observacao")
...
function StatCard({ label, value, sub, icon: Icon, accent }) {
  const accents = { green: "border-emerald-500", amber: "border-amber-400", blue: "border-para-blue" };
  const textAccents = { green: "text-emerald-600", amber: "text-amber-600", blue: "text-para-blue" };
  const bgAccents = { green: "bg-emerald-50", amber: "bg-amber-50", blue: "bg-blue-50" };

  return (
    <div className={`bg-white rounded-xl border border-slate-200 shadow-sm border-l-4 ${accents[accent]} p-5 flex items-start gap-4`}>
      <div className={`p-2.5 rounded-lg ${bgAccents[accent]} shrink-0`}>
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="flex items-end justify-between">
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Dashboard CPAG</h1>
            <p className="text-slate-400 text-sm font-medium mt-1">Controle de Pagamentos — Exercício Fiscal 2026</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatCard label="Total a Pagar" value={formatCurrency(kpis.totalAPagar)} sub="DLs sem OB correspondente" icon={Clock} accent="amber" />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_280px] gap-6">
        <div className="space-y-6 min-w-0">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-400 font-medium mt-0.5">Todos os exercícios — DLs sem OB emitida</p>
              </div>
              {liquidados.length > 0 && (
            ...
            {liquidados.length > 0 ? (
              ...
              <div className="px-6 py-16 text-center">
                ...
                <p className="text-slate-400 text-sm font-medium">Nenhum documento de liquidação a pagar encontrado.</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h2 className="text-sm font-black uppercase tracking-widest text-slate-800">Monitoramento de Pagamentos</h2>
                <p className="text-[11px] text-slate-400 font-medium mt-0.5">Ordens bancárias emitidas e status de confirmação</p>
              </div>
            ...
            {monitoramento.length > 0 ? (
                  ...
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      {["Processo", "Credor", "Fonte", "DL", "OB", "Data Pgto", "Valor", "Status"].map((h, i) => (
                        <th key={h} className={`px-5 py-3 text-[11px] font-black uppercase tracking-wider text-slate-500 ${i >= 6 ? "text-right" : i === 7 ? "text-center" : "text-left"}`}>
                          {h}
                        </th>
                      ))}
                    </tr>
              ...
              <div className="px-6 py-16 text-center">
                <p className="text-slate-400 text-sm font-medium">Nenhuma ordem bancária encontrada.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <aside className="space-y-4">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            <p className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-4">Relatórios</p>
            <CpagExportButtons />
          </div>

          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            <p className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-4">Painel de Indicadores</p>
            <div className="divide-y divide-slate-100">
              {[
                ...
                { label: "DLs sem OB", value: liquidados.length, color: "text-para-blue", bg: "bg-blue-50" },
              ].map(({ label, value, color, bg }) => (
                <div key={label} className="flex items-center justify-between py-3">
                  <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{label}</span>
                  <span className={`text-sm font-black ${color} ${bg} px-2.5 py-1 rounded-md`}>{value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-slate-900 rounded-xl p-5">
            <p className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-3">Observações</p>
            <p className="text-xs text-slate-300 leading-relaxed">
              O status de pagamento pode ser confirmado manualmente na coluna Status da tabela de Monitoramento.
            </p>
          </div>
        </aside>
      </div>
