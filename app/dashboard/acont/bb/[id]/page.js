...
export default async function BBContaDetailPage({ params, searchParams }) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Breadcrumb */}
      <div>
        <Link href={`/dashboard/acont/bb?ano=${ano}`} className="inline-flex items-center gap-1.5 text-[11px] font-bold text-slate-400 hover:text-para-blue uppercase tracking-widest mb-5 transition-colors">
          <ChevronLeft size={13} />
          Banco do Brasil
        </Link>
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm font-medium mt-0.5">
                BB · Ag. {conta.agencia} · {conta.numero_conta} · Exercício {ano}
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-[11px] font-black uppercase tracking-widest ${
              conf?.ok ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-amber-50 border-amber-200 text-amber-700"
            }`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div className={`bg-white rounded-xl border shadow-sm px-6 py-5 flex flex-col gap-2 ${Math.abs(disp - razao) > 0.05 ? "border-amber-200" : "border-slate-200"}`}>
          ...
          {Math.abs(disp - razao) > 0.05 && <p className="text-[11px] text-amber-500 flex items-center gap-1"><Info size={11} />Diferença: {formatCurrency(Math.abs(disp - razao))}</p>}
        </div>
        <div className={`bg-white rounded-xl border shadow-sm px-6 py-5 flex flex-col gap-2 ${extCC != null && Math.abs(disp - extCC) > 0.05 ? "border-red-200" : "border-slate-200"}`}>
          ...
          {extCC != null && Math.abs(disp - extCC) > 0.05 && <p className="text-[11px] text-red-400 flex items-center gap-1"><Info size={11} />Diferença: {formatCurrency(Math.abs(disp - extCC))}</p>}
        </div>
      </div>
