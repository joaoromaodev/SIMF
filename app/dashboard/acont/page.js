...
async function fetchKpisBanco(supabase) {
  const { data, error } = await supabase
    ...
    .select("banco, qtd_contas, total_disponibilidade, total_razao");
  if (error) return [];
  return data || [];
}
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-7 py-6 flex flex-col gap-2">
          <p className="text-xs font-black uppercase tracking-widest text-slate-400">Total de Contas Ativas</p>
