...
export default async function AcontBanparaPage({ searchParams }) {
  const sp        = await searchParams;
  const ano       = sp.ano || "2026";
  const supabase  = getSupabaseAdminClient();
  const contas    = await fetchContas(supabase, "BANPARA", ano);
      ...
      <div>
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-black text-slate-900 tracking-tight">Banpará</h1>
          ...
          <div className="flex-shrink-0">
            <AcontConsolidadoExportButtons banco="BANPARA" exercicio={ano} />
          </div>
        </div>
      </div>

      <AcontBankList contas={contas} banco="BANPARA" slug="banpara" ano={ano} />
    </div>
  );
}
