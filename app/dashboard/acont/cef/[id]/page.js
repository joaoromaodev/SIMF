...
export default async function CEFContaDetailPage({ params, searchParams }) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link href={`/dashboard/acont/cef?ano=${ano}`} className="inline-flex items-center gap-1.5 text-[11px] font-bold text-slate-400 hover:text-para-blue uppercase tracking-widest mb-5 transition-colors">
          <ChevronLeft size={13} />
          Caixa Econômica
        </Link>
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-orange-50 rounded-lg"><Landmark size={20} className="text-orange-600" /></div>
            <div>
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">{conta.finalidade}</h1>
              <p className="text-slate-400 text-sm font-medium mt-0.5">
                CEF · Ag. {conta.agencia} · {conta.numero_conta} · Exercício {ano}
