...
export default async function CEFContaDetailPage({ params }) {
  const { id } = await params;
  const supabase = getSupabaseAdminClient();
  const { conta, saldos, extrato } = await fetchContaDetalhe(supabase, id);

  if (!conta || conta.banco !== "CEF") notFound();
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link href="/dashboard/acont/cef" className="inline-flex items-center gap-1.5 text-[11px] font-bold text-slate-400 hover:text-para-blue uppercase tracking-widest mb-5 transition-colors">
          <ChevronLeft size={13} />
          Caixa Econômica
        </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm font-medium mt-0.5">
                CEF · Ag. {conta.agencia} · {conta.numero_conta}
                {conta.conta_contabil && <><span className="text-slate-300 mx-2">·</span><span className="font-mono text-[11px]">{conta.conta_contabil}</span></>}
              </p>
            </div>
          </div>
      ...
      <div>
        <p className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-4">Detalhamento por Fonte</p>
