...
export default async function AcontCEFPage({ searchParams }) {
  const sp        = await searchParams;
  const ano       = sp.ano || "2026";
  const supabase  = getSupabaseAdminClient();
  const contas    = await fetchContas(supabase, "CEF", ano);
      ...
      <div>
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex-shrink-0">
            <AcontConsolidadoExportButtons banco="CEF" exercicio={ano} />
          </div>
        </div>
      </div>

      <AcontBankList contas={contas} banco="CEF" slug="cef" ano={ano} />
    </div>
  );
}
